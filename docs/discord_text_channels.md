...
Trackers prefer an authorized BattleMetrics API snapshot when the operator supplies a working token. If it is unavailable, a SteamID-based tracker checks the player's public Steam Join Game endpoint against the target server, then uses direct public A2S, a hosted A2S transport retry, and finally a local snapshot no older than three minutes. A hidden Steam endpoint remains unknown. A2S does not expose Steam IDs, so duplicate names stay ambiguous and are never converted into permanent identity links.
